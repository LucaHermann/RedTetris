...
export class Room {
  ...
  public generateSpectrum(grid: number[][], user: IUser): void {
    ...
    this.room.spectrums[idSpectrum].spectrum = Array.from({ length: 20 }, () =>
      Array(10).fill(0)
    );
    const i: number[] = [];
    if (idSpectrum >= 0) {
  ...
  public removePlayer(user: IUser): void {
    ...
    this.room.players.length < 1
      ? // tslint:disable-next-line: no-unused-expression
        null
      : (this.room.gameMaster = this.room.players[0]);
  }
